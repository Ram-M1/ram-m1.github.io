...
window.fbTouchOnline = async function(force, online) {
  ...
  try {
    ...
    try {
      ...
      /* ВНУТРЕННИЙ КОД FOCUS — как @имя в Telegram.
...
window.fbFindUserByEmail = async function(email) {
  ...
  if (!user) return { ok: false, error: 'Не авторизован' };
  const em = String(email || '').trim().toLowerCase();
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(em)) return { ok: false, error: 'Введи корректную почту' };
  try {
    const base = (window.FOCUS_AI_PROXY || '').replace(/\/+$/, '');
    const r = await fetch(base + '/users/find', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ uid: user.uid, mode: 'email', q: em })
    });
    const d = await r.json();
    if (!d.ok) return { ok: false, error: d.error || 'Не найден' };
    return { ok: true, user: d.user, users: d.users || [d.user] };
      ...
      if (peers.length && window.fbGetProfilesBatch) {
        ...
        list.forEach(function(c){
          ...
          if (p) {
            ...
            /* ИМЯ ВСЕГДА ИЗ ВИЗИТКИ. Раньше сохранённая заглушка «Пользователь»
               оставалась навсегда, даже когда настоящее имя уже известно. */
            if (p.name) c.withName = p.name;
            c._online = p.online;
            c._lastSeen = p.lastSeen;
            c._live = true;
          } else if (c.type !== 'group') {
            /* ЗАПИСЬ-ПРИЗРАК: живого профиля за собеседником нет. Такие остались от
               прежнего способа входа (у человека сменился внутренний номер аккаунта)
               и показывались ДУБЛЯМИ рядом с настоящим контактом — без фото и без имени. */
            c._live = false;
          }
        });
      }
