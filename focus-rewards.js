...
(function () {
  ...
  /* НАЧИСЛЕНИЕ МОНЕТ ЗДЕСЬ ОТКЛЮЧЕНО — и это правильно.

     Монеты за задания начисляются НЕ здесь, а по официальному плану в status.html:
       зарядка 5 дней = 70, сон 7 дней = 70, благодарность = 63,
       висяки = 30, 3 активных друга = 90, бонус за полную неделю = 10  →  ИТОГО 333 F-coin
     Ровно столько стоит подписка Лайт. Плюс путь B: друг купил подписку → тебе столько же монет.

     Эти монеты — ЗАРАБОТАННЫЕ: их можно потратить ТОЛЬКО на подписку.
     ИИ-запросы оплачиваются исключительно КУПЛЕННЫМИ монетами (правило в focus-storage).

     Здесь mark() только фиксирует активность и стрики (для Прогресса) — без монет,
     иначе экономика поехала бы: юзер получал бы монеты дважды за одно и то же. */
  var MODE = 'none';
  ...
  var FocusRewards = {
    ...
    coinBurst: function (config) {
      ...
      for (var i = 0; i < count; i++) {
        ...
        el.style.transition = 'none';   // монеты двигаем покадрово — переход тут только мешает
      ...
      function animate() {
        ...
        if (frame === 70) {
          // ВАЖНО: снимаем плавный переход перед покадровым управлением.
          // Раньше transition и ручные изменения стиля конфликтовали — надпись дёргалась и мелькала.
          note.style.transition = 'none';
        }
        if (frame === 70) note.style.transition = 'none';   // снимаем переход — иначе он дерётся с покадровой анимацией (мелькание)
        if (frame > 70) {
