...
(function(){
  ...
  function openSearchPeople(){
    showSheet(
      ...
      '<div class="sheet-sub">Введи номер телефона — так же, как в WhatsApp.</div>' +
      '<input class="sheet-input" id="findInput" type="tel" inputmode="tel" placeholder="+7 917 963-07-77" autocomplete="off">' +
    ...
    async function doFind(){
      ...
      try {
        /* ПОИСК ТОЛЬКО ПО НОМЕРУ — как в WhatsApp.
           Поиск по имени отдавал карточки незнакомых людей вместе с их настоящими
           номерами, а промах запускал перебор тысяч записей на сервере. Номер
           обязателен при заполнении анкеты, значит по нему находится любой. */
        var matches = [];
        var digits = q.replace(/\D/g, '');
        if (digits.length < 10) {
          box.innerHTML = '<div class="empty" style="padding:20px 14px;"><div class="empty-s">Введи полный номер телефона</div>' +
            '<div style="font-size:11.5px;color:#8e8e9e;margin-top:8px;line-height:1.5;">Например: +7 917 963-07-77</div></div>';
          btn.disabled = false; btn.textContent = 'Найти';
          return;
        }
        if (window.fbFindUserByPhone) {
        ...
        if (!matches.length) {
          box.innerHTML = '<div class="empty" style="padding:20px 14px;"><div class="empty-s">Никого не найдено</div>' +
            ...
            'Этого номера нет в FOCUS. Проверь, верно ли он набран — или позови человека в приложение.</div></div>';
        }
