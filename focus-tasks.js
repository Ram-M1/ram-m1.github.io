/* ═══ СВЯЗКА ТРЕНИРОВКИ ФОКУСА С РЕАЛЬНЫМИ ДЕЛАМИ ═══
   ...
   Что происходит после сессии:
     ...
     • НЕ ДОДЕЛАЛ → просим комментарий, задача уходит в «Доделать» и хранит его.
...
(function () {
  ...
  var TODO_KEY = 'focus_focus_todo';    // «Доделать»
  ...
  /** Сессия закончилась, дело СДЕЛАНО: в завершённые, галочка в источник,
      и убираем из «Доделать», если оно там висело. */
  ...
  /** Убрать из «Доделать» без завершения (человек передумал). */
  function dropUnfinished(key) {
    write(TODO_KEY, todoList().filter(function (t) { return t.key !== key; }));
  }

  /** Источник задачи мог быть удалён, пока она висела в «Доделать».
